...
.bc-tariff-popup .tariff-popup__radio [an-la*="tariff:select"]`);
        ...
        this.cbxTermsAndConditions = page.locator(`.tariff-popup__checkbox:has(> input[required])`);
    ...
    async addSIMPopupInBC() {
        try {
            ...
            if (await this.cbxTermsAndConditions.isVisible({ timeout: 2000 })) {
                await this.utils.scrollClick(this.cbxTermsAndConditions);
            }
