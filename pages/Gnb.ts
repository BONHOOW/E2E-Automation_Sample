...
export class Gnb {
  ...
  private static readonly gnbSelectors = {
    ...
    shopCatalog: (type: string, v: string) => `.nav-level2-column-wrapper a[data-an-la="${type}: "i][href*="${v}"i]`
  };
  ...
  async hoverGnbCategory(text: string) {
    try {
      ...
      console.log('✓ Waited for category to appear after hover');
    } catch (error: any) {
  ...
  public async clickCart(): Promise<void> {
    try {
      ...
      // Use Playwright standard method: expect.poll()
      let clickableButton: Locator | null = null;
      
      await expect.poll(async () => {
        const cartButtons = await this.btnCart.all();
        for (const button of cartButtons) {
          if (await button.isVisible() && await button.isEnabled()) {
            clickableButton = button; // Store in external variable
            return button; // Return found button
          }
        }
        return null; // Return null if not found
      }, {
        timeout: 5000,
        message: 'Waiting for visible and enabled cart button'
      }).not.toBeNull();
      
      // Click found button
      if (clickableButton) {
        await (clickableButton as Locator).click();
      }
  ...
  // async clickRandomSecondaryNavi(): Promise<{ text: string; href: string }> {
