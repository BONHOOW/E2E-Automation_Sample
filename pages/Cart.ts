...
export class Cart {
  readonly page: Page;
  readonly ssflexPage: SSFlex;
  private tradeInData: any;
  readonly scpPage: SCPPage;
  ...
  readonly ssflexAddBtn: Locator;    // SSFlex add button locator


  constructor(page: Page, tradeInData?: any) {
    this.page = page;
    ...
    // Delete icon (Samsung Care+ item delete button)
    ...
    // SSFlex add button locator
    this.ssflexAddBtn = page.locator(`[data-an-la="add service:samsung flex"]`);
...
.cart-item-list.ng-star-inserted:nth-child(${line} of .cart-item-list) .service-item[data-modelcode="TRADE-IN"]:not(.service-item__trade-up) button[data-an-la="add service:trade-in"]`;
  ...
  async addSCPlus(line: number, type: string) {
    try {
      ...
      const scpAddBtn = `.cart-product-list .cart-row:nth-child(${line}) [data-an-la="add service:samsung care"],
    .cart-item-list [data-an-la="add service:samsung care"],
    .cart-item-list.ng-star-inserted:nth-child(${line} of .cart-item-list) [data-an-la="add service:samsung care"]`;

      await this.page.locator(scpAddBtn).click();
      await this.scpPage.addSCPOptionsInCart(type);
  ...
  async addSSFlex() {
    try {
      // Click SSFlex add button for the corresponding SKU object
      await this.ssflexAddBtn.scrollIntoViewIfNeeded();
      this.ssflexAddBtn.click();
      await this.ssflexPage.addSSFlexInCart();

    } catch (error: any) {
      throw new Error(`Failed to add Samsung Flex: ${error.message}`);
    }
  }

  async verifySSFlexInCart() {
    try {
      await this.ssflexPage.verifySSFlexRemoveBtn();
      return true;

    } catch (error: any) {
      throw new Error(`Samsung Flex verification failed: ${error.message}`);
    }
  }
...
`
  ...
  private btnRemoveAddedSCP = (line: number) => {
    const selectors = [
     `.cart-product-list .cart-row:nth-child(${line}) [data-an-la="add service:samsung care"],
...
.cart-item-list.ng-star-inserted:nth-child(${line}) [data-modeldisplay="Samsung Care"] [data-an-la="remove item"],
.cart-item-list.ng-star-inserted:nth-child(${line}) [data-modeldisplay*="Samsung Care+"] [data-an-la="remove item"],
.cart-item-list .service-item__smc [data-an-la="remove item"],
[data-pvisubtype="samsung care"] button[data-an-la="remove item"],
.service-item [data-an-la="remove item"]`
  ];
    return selectors.join(', ');
    };
...
.cart-item-list.ng-star-inserted:nth-child(${line} of .cart-item-list) .service-item[data-modelcode="TRADE-IN"]:not(.service-item__trade-up) button[data-an-la="add service:trade-in"]`;
  ...
  /**
   * Verify that multiple SKUs are in cart (similar to Katalon's verifyAreSkusInCart)
  ...
  async isAddSCPlusAvailable(line: number): Promise<boolean> {
    const scpAddBtn = `.cart-product-list .cart-row:nth-child(${line}) [data-an-la="add service:samsung care"],
    .cart-item-list :nth-child(${line}) [data-an-la="add service:samsung care"],
    .cart-item-list.ng-star-inserted:nth-child(${line} of .cart-item-list) .service-item[data-modelcode="SAMSUNG CARE"] button[data-an-la="add service:samsung care"]`;
  ...
  async addSCPlusTryBothOptions(): Promise<void>{
    try{
      ...
      try{
        ...
        throw new Error('Failed to add Samsung Care+ in cart');
      }
    }
  }

    /**
     * Verify that Samsung Care+ is added to cart through price area
     */
    async verifySCPInCart(line: number) {
      try {
        const scpRemoveButton = this.page.locator(this.btnRemoveAddedSCP(line));
        await expect(scpRemoveButton).toBeVisible({ timeout: TIMEOUTS.LONG });
      } catch (error: any) {
        throw new Error(`Samsung Care+ verification failed: ${error.message}`);
      }
    }
  ...
  async isAddSSFlexAvailable(): Promise<boolean> {
    try {
      await this.ssflexAddBtn.scrollIntoViewIfNeeded();
      await this.ssflexAddBtn.waitFor({ timeout: TIMEOUTS.SHORT });
      return await this.ssflexAddBtn.isEnabled();
    } catch {
      return false;
    }
  }
