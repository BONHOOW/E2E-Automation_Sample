...
export class BC {
  ...
  private optionSSFlex: Locator;
  private btnUpgradeApply: Locator;
  

  constructor(page: Page, tradeInData?: any, config?: any) {
    this.page = page;
    this.utils = new Utils(page);
    this.scpPage = new SCPPage(page);
    this.tradeInData = tradeInData;
    this.config = config;
    this.addonPage = new AddonPage(page);
    this.cart = new Cart(page);
    this.simPage = new SIMPage(page);
    this.ssflexPage = new SSFlex(page);
    this.areaTradeIn = this.page.locator('.hubble-product__options.s-option-trade, .wearable-option.trade-in');
    this.areaSCP = this.page.locator('.hubble-product__options#samsung-care, div.wearable-option.option-care');
    this.areaGalaxyClub = this.page.locator('.hubble-product__options.s-option-galaxy-club');
    ...
    this.optionSCPNone = this.page.locator('.hubble-product__options-list.care_option_list.is-delete, .hubble-product__options.s-option-care #carenone, .wearable-option__select-item button[an-la="samsung care:none"]');
    this.optionGalaxyClubNo = this.page.locator('.hubble-product__options.s-option-galaxy-club #gc-no-btn');
    this.optionGalaxyClubYes = this.page.locator('.hubble-product__options.s-option-galaxy-club #gc-yes-btn');
    ...
    this.optionSSFlex = this.page.locator(`:has(> [data-service-type="UPGRADE"i]),
.option-purchase [an-la="purchase program:samsung flex"]`);
    this.btnUpgradeApply = this.page.locator(`[an-la="upgrade program:apply"],
[an-la="purchase option:samsung flex:add"]`);
  }
  ...
  async selectBookOptions(options: {
    ...
    try {
      ...
      // Note: Memory and graphics are commented out in Katalon, so we'll skip them for now
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
  ...
  async verifyPriceValues(target: Locator): Promise<string[]> {
    try {
      ...
      if (BC.PRICE_EXCLUDED_SITES.includes(site?.toLowerCase() as any)) {
        return [];
      }
  ...
  async isSCPAvailable(): Promise<boolean> {
    try {
      return await this.areaSCP.isVisible({ timeout: 3000 });
    } catch {
      return false;
    }
  }
  ...
  // async addToCart() {
  //   try {
  //     console.log('Step: Adding to cart...');
      
  //     // Wait for button to be enabled
  //     await expect(this.btnAddToCart).toBeEnabled({ timeout: TIMEOUTS.STANDARD });
  //     console.log('✓ Add to Cart button is enabled');

  //     await this.utils.scrollClick(this.btnAddToCart);
...
async continueToCart(): Promise<void> {
  try {
    ...
    await this.scpNoneIfNeeded();
    await this.galaxyClubNoIfNeeded();  
...
async skipBridge(): Promise<void> {
  ...
  async tradeInNoIfNeeded(): Promise<void> {
    if (await this.areaTradeIn.isVisible({ timeout: 15000 })) {
      ...
      try {
        ...
        const isAlreadyAdded = await this.btnRemoveAdeedTradeIn.isVisible({ timeout: 3000 });
        if (!isAlreadyAdded) {
  ...
  async galaxyClubNoIfNeeded(): Promise<void> {
    if (await this.areaGalaxyClub.isVisible({ timeout: 15000 })) {
      try {
        // Check if both Yes and No buttons are disabled
        const isYesDisabled = await this.optionGalaxyClubYes.isDisabled();
        const isNoDisabled = await this.optionGalaxyClubNo.isDisabled();
        
        // If both buttons are disabled, skip selection
        if (isYesDisabled && isNoDisabled) {
          console.log('Galaxy Club options are disabled, skipping selection');
          return;
        }
        
        // Check if "Yes" is selected
        const isYesSelected = await this.optionGalaxyClubYes.evaluate(el => {
  ...
  async scpNoneIfNeeded(): Promise<void> {
    if (await this.areaSCP.isVisible({ timeout: 15000 })) {
      console.log('SCP area found, checking status...');
      try {
        // Check if any SCP option (other than None) is selected
        const hasSelectedOption = await this.areaSCP.locator(':not(.is-delete)').evaluateAll(elements => {
        ...
        // If no SCP option is selected, select "None"
        if (!hasSelectedOption) {
          console.log('No SCP option selected, selecting "None"');
          await this.selectNoSCP();
        } else {
          console.log('SCP option already selected, skipping');
        }
      } catch {
        // If error occurs, assume not selected and proceed with "None" selection
        console.log('Error checking SCP status, selecting "None"');
        await this.selectNoSCP();
      }
    } else {
      console.log('No SCP area found, skipping');
    }
  }
  ...
  async selectNoSCP() {
    ...
    try {
      await expect(this.areaSCP).toBeVisible({ timeout: TIMEOUTS.LONG });
    } catch (error) {
      // Skip if not visible
      console.log('SCP section not visible, skipping');
      return;
    }
    
    // If we reach here, SCP is visible
    // Throw custom error if No button click fails
    try {
      await this.utils.scrollClick(this.optionSCPNone);
      console.log('No SCP option selected');
    } catch (error) {
      throw new Error('Failed to select no SCP option');
    }
  }

  async addSamsungCarePlus() {
    // Fail if SCP area is not visible
    try {
      await expect(this.areaSCP).toBeVisible({ timeout: TIMEOUTS.LONG });
    } catch (error) {
      throw new Error('SCP section is not visible - cannot add Samsung Care+');
    }
    
    // Throw custom error if Samsung Care+ addition fails
    try {
      await this.areaSCP.scrollIntoViewIfNeeded();
      console.log('Samsung Care+ section is visible. Adding Samsung Care+...');
      await this.scpPage.addSCPOptionsInBC();
      await this.scpPage.addSCPPopupInBC();
      console.log('Samsung Care+ addition completed in BC page.');
    } catch (error) {
      throw new Error('Failed to add Samsung Care+ in BC page');
    }
  }

  
   async selectNoGalaxyClub() {
    // Check if visible first
    try {
      await expect(this.areaGalaxyClub).toBeVisible({ timeout: TIMEOUTS.LONG });
    } catch (error) {
      // Skip if not visible
      console.log('Galaxy Club section not visible, skipping');
      return;
    }
    
    try {
      await this.utils.scrollClick(this.optionGalaxyClubNo);
      console.log('No Galaxy Club option selected');
    } catch (error) {
      throw new Error('Failed to select no Galaxy Club option');
    }
  }
  ...
  /**
   * Check if SCP "Yes" option is already selected
   * @returns {Promise<boolean>} True if any SCP option (other than None) is selected
   */
  private async isSCPYesSelected(): Promise<boolean> {
    try {
      if (!(await this.areaSCP.isVisible({ timeout: 3000 }))) return false;
  ...
  private getTradeInDataForDevice(device: string) {
    // Set phone as default
    let data = this.tradeInData?.phone;
    let deviceType = 'phone';
    
    // Use corresponding data if watch or tablet is detected
    ...
    console.log(`Trade-in data:`, JSON.stringify(data, null, 2));
  ...
  async verifyTradeInAdded(): Promise<void> {
    try {
      ...
      // Web-first assertion - Playwright handles auto-waiting and scrolling
  ...
  async addSSFlex() {
    await this.utils.scrollClick(this.optionSSFlex);
    await this.clickUpgradeApply();
    await this.ssflexPage.waitForOpenedSSFlexPopup();
    await this.ssflexPage.clickConfirmSSFlexPopup();
  }

  async isSSFlexAvailable(): Promise<boolean> {
    try {
      return await this.optionSSFlex.isVisible({ timeout: TIMEOUTS.LONG });
    } catch {
      return false;
    }
    return this.optionSSFlex.isVisible({ timeout: TIMEOUTS.SHORT });
  }
